...
`       See the top of the file for details.
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        obs_ph = observations_ph.get()
        if isinstance(obs_ph, dict):
            obs_keys = list(obs_ph.keys())
            batch_size = tf.shape(obs_ph[obs_keys[0]])[0]
        else:
...
`       See the top of the file for details.
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        # ！！！这里q_t的参数和上面act_f中的q_func的参数是一样的
        # Q(st, a)
        q_t = q_func(obs_t_input.get(), num_actions, scope="q_func", reuse=True)  # reuse parameters from act
        q_func_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=tf.get_variable_scope().name + "/q_func")

        # target q network evalution
        # 注意这里输入的obs与上面不同，应该是t+1时刻的，上面的是t时刻的
        # Q^(st+1, a)
        ...
        # 当episode结束时，maxQ^(st+1, a)不存在，所以设为0
