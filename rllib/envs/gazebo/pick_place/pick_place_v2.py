...
class PickPlacev2Env(gazebo_env.GazeboEnv):
    def __init__(self, step_type, grab_range_z_mm, random_xy_type):
        ...
        self.observation_space = spaces.Dict(dict(
            ...
            obj_dist = spaces.Box(-np.inf, np.inf, shape=(DOF,), dtype='float32')
        ))
    ...
    def step(self, action):
        ...
        if not is_collision and self._enter_grab_range():
            reward = 1
            done = False
    ...
    def _get_obs(self):
        ...
        obs = {'img_left1' : img_left1,
               ...
               'obj_dist': self.gripper_pos -
                           (self.object_pos-self.robot_base+np.array([0]*5+[OBJECT_H_MM+self.grab_range_z_mm]))
               }
        return obs
