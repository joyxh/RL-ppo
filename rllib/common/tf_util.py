...
def function(inputs, outputs, updates=None, givens=None):
    ...
    四个参数用法：
    inputs: 需要外部赋值的placeholders（tensors)
    outputs: 图的输出tensors
    updates: 通常图的主体部分在运行时会用到一些变量，这些变量有自己的更新逻辑，
             这部分更新逻辑需要通过updates来强制执行，但不会作为output返回
    givens: inputs的默认参数，当inputs缺失时使用
...
class _Function(object):
    def __init__(self, inputs, outputs, updates, givens):
        for inpt in inputs:
            # 'make_feed_dict'参考deepq/utils/TfInput
        ...
        # 将输出tensor和需要更新的tensor合并
        # 后面会通过session.run一起运行
    ...
    def __call__(self, *args):
        ...
        for inpt in self.givens:
            # 如果inpt存在于feed_dict，则把feed_dict[inpt]赋值给feed_dict[inpt]
            # 否则，在feed_dict中插入self.givens[inpt]
            # 所以givens就是默认参数，如果外部输入参数，则以外部参数为准
            feed_dict[inpt] = feed_dict.get(inpt, self.givens[inpt])
        # 运行图，但是注意不返回updates包含的tensor
